	...
	if (!NETWORK::NETWORK_IS_GAME_IN_PROGRESS() && MISC::GET_GAME_TIMER() - iLocal_55 > 2000 || NETWORK::NETWORK_IS_GAME_IN_PROGRESS() && NETWORK::IS_TIME_MORE_THAN(NETWORK::GET_NETWORK_TIME(), NETWORK::GET_TIME_OFFSET(iLocal_56, 2000)) || bLocal_58)
	{
		...
		for (j = 0; j < num3; j = j + 1)
		{
			...
			if (func_59(echLocal_57, num2, outValue, &value))
			{
				...
				{
					if (Global_114162.f_2367.f_539.f_2387[echLocal_57] == PV_COMP_HEAD)
						flag2 = false;
...
BOOL func_9() // Position - 0x841 (2113)
{
	...
	if (Global_2685663.f_2847.f_26 == true)
		return false;
...
void _STOPWATCH_INITIALIZE(int pStopwatch, BOOL useLocalTimer, BOOL useAccurateTime) // Position - 0x1068 (4200)
{
	if (pStopwatch->f_1 == 0)
	{
...
BOOL func_49(ePedComponentType epctParam0) // Position - 0x125A (4698)
{
	if (func_50(epctParam0))
		if (func_50(Global_1887694[epctParam0 /*611*/].f_10))
...
BOOL _IS_MISSION_REPEAT_ACTIVE(BOOL bExcludeBenchmark) // Position - 0x150B (5387)
{
...
{
	return Global_113247 && bParam0 != false;
}
